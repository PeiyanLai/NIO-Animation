 ...
 *  ⚠️ 照片锚定控件，位置经过一次用户纠偏（2026-08-19）：
 *  正确位置是**储物盖板前缘的槽形把手**（软包段起点前 ≈15 世界单位、阶梯上沿），
 *  俯视对应扶手盖板前缘中央把手——不是托盘前段那个椭圆键（旧值 190.5,223 已废）。
 *  本值由用户截图红箭头经锚点换算得来：以两个锁舌插槽圆点为锚解 截图→舞台 映射，
 *  再按 SIDE_CAM 反解世界坐标（过程见会话记录）。**换相机必须重反解**。 */
export const BUTTON = {
  x: 397.6, y: 197.9,
  w: smm(70), h: smm(16),
};
