...
### 换图带来的连锁问题：原来靠什么区分的，换完还在吗

统一车图会**吃掉一条叙事通道**。对讲机动画原先靠车身颜色（金 / 蓝灰）区分
「蔚来车 / 非蔚来车」，换成实拍图后四台都是黑车，颜色通道没了；
试过给非蔚来车加去色滤镜，但**对黑车等于没做**。最后改用**轮廓描边色**——
换图前先想清楚「这一版靠什么区分」，别等渲出来才发现读不出来。
...
判据也不能用单侧绝对阈值：**白车对浅灰底只差 10–15 级**，`lum < 固定值` 直接失效。
要用「偏离该行/列**背景基线** ±（亮 9 / 暗 25）**且连续两像素**」——双向是因为
车身既有比背景亮的白漆，也有比背景暗的黑唇黑格栅；连续两像素是为了滤掉噪点。
背景基线的取样窗要确认**没被主体污染**（扫机盖时窗口开在 y=110–140，开在 150–170 就会
在车头高的那几列取到车身本身，结果一路乱跳）。

扫完**必须做移动平均再抽样**，否则 ±1px 的抖动会在轮廓上留下肉眼可见的锯齿——
这正是「曲线不够完美」的来源。但天线、扰流板这类真实凸起不能被平滑抹平，
那一段要单独用小窗口。
