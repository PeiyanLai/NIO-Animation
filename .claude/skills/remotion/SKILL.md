...
## 交付闭环（强制）：把 HTML 文件本体交到用户手里

动画定稿后，交付物 = **单文件交互 HTML（右上角内嵌各章高清 GIF 一键下载）**。
必须让用户**拿到文件本身**，按下面的降级序列执行，走到哪级就明说哪级——
绝不允许只贴代码块、只报一句「已生成」、或只留在工作目录里不吭声：

1. **有文件发送/附件工具**（如 Claude Code 环境的 `SendUserFile`，
   `display: 'render'` 可侧栏直开）→ 把 `dist/xxx-player.html` 直接发进对话，
   附一句使用说明：**点击画面暂停/继续、顶部 chips 切章节、右上角一键下载
   各章高清 GIF**——不说没人会去点。
2. **没有附件工具，但平台能生成文件下载链接**（工作区文件服务、制品导出等）→
   给出 HTML 的下载链接 + 同一句使用说明。
3. **两者都没有** → 把 HTML 放到用户可访问的固定输出目录，回复里给出
   **绝对路径和取用方法**，并明确说「这个 HTML 文件就是唯一交付物，
   请下载后用浏览器打开」。

**开工前先自检交付能力（强制）**：列一遍自己可用的工具，确认走 1/2/3 哪条路。
干完活才发现发不出文件 = 交付失败。渲染环境同样先自检：
...
不存在「能出 HTML 但出不了 GIF」的中间态。有红 = 本机不能渲染，
HTML 由能跑渲染的一方产出后交给你分发——这个分工可行，但要明说。

**不再默认生成飞书文档**（2026-08-19 用户裁定）：用户拿到 HTML 后自己决定去向，
页面右上角的 GIF 下载入口已覆盖「贴飞书」的需求。GIF 质量线不因此放松：
1440×810（`--scale 0.75`）/ 6fps 是及格线，省体积靠降帧率不靠降分辨率
（1080p30 直渲会 OOM，`--concurrency=2`）。
仅当用户明确点名「要飞书文档」时，才走 `references/feishu-delivery.md` 的流程
（文档正文平铺 GIF image 块、HTML 传 file 块防签名 URL 403 等坑都记录在那份文档里）。

## 每轮交付后的复盘沉淀（强制）

**每完成一次动画制作、每处理完一轮人工反馈修改，交付闭环的最后一步是复盘**：
花几分钟回答下面四个问题，有产出就提交进仓库——skill 是靠这个机制变好用的，
不是靠一次写对。

1. **踩了什么新坑 / 用了什么新方法？** → 写进 SKILL.md 对应章节，
   **必须带本轮的真实案例**（错误现象 + 正确做法），没有案例支撑的经验不收。
2. **是不是长期规则？** 判据两条：用户说了「以后都要…」，或同类错误第二次出现。
   是 → 升级进 `references/hard-rules.md` 清单（并在 SKILL 正文留详细版）。
3. **产生了新素材 / 新标定吗？** → 按素材纪律入库：photos/ + manifest 登记，
   可复用的进 assets/ 资产库并保持同步校验。
4. **返工的根因是不是需求描述不清？** → 把教训补进需求描述指南,
   下次需求文档就能提前堵住。

沉淀本身也走版本闭环：提交推送到工作分支，里程碑随代码一起推平 main——
公司 agent 下次 update.py 就学到了。

**防膨胀纪律**（沉淀不是越多越好）：
- 只沉淀**可执行、可检查**的条目；「要注意质量」这种空话不收。
- 一次性的项目细节（某个动画的坐标、配色值）留在该动画的数据层注释里，
  不进 skill——skill 收的是**方法和判据**，不是参数。
- 同一条规则不写两遍：hard-rules.md 放一行索引,SKILL 正文放唯一的详细版。
...
## 信息卡四角禁区是全局规则（强制,所有动画所有图层）

信息卡/字幕块/步骤块**整体不得贴靠画面四角**(bag-data.CORNERS 的四块:
190×105 的四个角区),SVG 层和 HTML 层都算;只有「视角标签」这类非信息装饰允许留角上。
每个新动画要么复用 CORNERS 断言,要么在布局注释里写明已避开。
拆给 agent 做的动画,**任务书必须附上这批布局硬禁区(四角/SAFE/贴主体带宽)**——
