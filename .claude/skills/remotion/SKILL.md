...
### 同一台车换配色：对齐轮心整体搬遮罩，别重描

拿到同车型的另一张同视角照片时，**不要重新描轮廓**——那是几小时的活，而且描不准。
做法是把新图**重采样进旧图的画幅**：
...
4. 按逆仿射把新图采样进旧画幅 → **`CAR_BODY`、轮拱、所有锚点、下游运动学常量全部沿用**

实测：白车前轮 (165,310) 后轮 (722,310) → 旧图 (196,318)/(770,318)，尺度 1.0305，
一次对齐后旧轮廓直接可用，只需再内缩 3px。
